Public Class Frm_TMX
    ...
    Private Sub Timer1_Tick(sender As Object, e As EventArgs) Handles Timer1.Tick
        ...
        TextBox2.Text = DataSet17.Tables("PartCount").Rows(0).Item(4)

        Dim opnofound As Integer = InStr(Trim(DataSet17.Tables("ProgramComment").Rows(0).Item(5)), ":", CompareMethod.Text) + 1
        TextBox10.Text = Mid((Trim(DataSet17.Tables("ProgramComment").Rows(0).Item(5))), opnofound, 4)
        Dim jobnofound As Integer = InStr(opnofound, Trim(DataSet17.Tables("ProgramComment").Rows(0).Item(5)), ":", CompareMethod.Text) + 1
        TextBox9.Text = Mid((Trim(DataSet17.Tables("ProgramComment").Rows(0).Item(5))), jobnofound, 5)



        'TextBox9.Text = Mid((Trim(DataSet17.Tables("ProgramComment").Rows(1).Item(4))), 1, 5) ' job number
        'TextBox10.Text = Mid((Trim(DataSet17.Tables("ProgramComment").Rows(1).Item(4))), 7, 4) ' operation number


        TextBox3.Text = DataSet17.Tables("Load").Rows(1).Item(4)
        TextBox11.Text = DataSet17.Tables("Load").Rows(3).Item(4)
        Try
            Spindleload = 419 - Int(DataSet17.Tables("Load").Rows(1).Item(4) * 3)
        Catch ex As Exception
            'MsgBox(ex.Message)
        End Try
        Try
            S2load = 419 - Int(DataSet17.Tables("Load").Rows(3).Item(4) * 3)
        Catch ex As Exception
            S2load = 419
            TextBox11.Text = "0"
            'MsgBox(ex.Message)
        ...
        TextBox4.Text = DataSet17.Tables("Load").Rows(4).Item(4)
        Xaxisload = 419 - Int(DataSet17.Tables("Load").Rows(4).Item(4) * 3)
        TextBox5.Text = DataSet17.Tables("Load").Rows(0).Item(4)
        Yaxisload = 419 - Int(DataSet17.Tables("Load").Rows(0).Item(4) * 3)
        TextBox6.Text = DataSet17.Tables("Load").Rows(6).Item(4)
        Zaxisload = 419 - Int(DataSet17.Tables("Load").Rows(6).Item(4) * 3)

        TextBox7.Text = DataSet17.Tables("PathFeedrateOverride").Rows(0).Item(5)
        ...
        End Select
        TextBox8.Text = DataSet17.Tables("PathFeedrateOverride").Rows(1).Item(5)
        ...
        End Select
        TextBox13.Text = DataSet17.Tables("EmergencyStop").Rows(0).Item(4)
