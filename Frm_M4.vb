Public Class Frm_M4
    ...
    Private Sub Timer1_Tick(sender As Object, e As EventArgs) Handles Timer1.Tick
        ...
        TextBox10.Text = Mid((Trim(DataSet21.Tables("ProgramComment").Rows(1).Item(4))), opnofound, 4)
        Dim jobnofound As Integer = InStr(opnofound, Trim(DataSet21.Tables("ProgramComment").Rows(1).Item(4)), ":", CompareMethod.Text) + 1
        TextBox9.Text = Mid((Trim(DataSet21.Tables("ProgramComment").Rows(1).Item(4))), jobnofound, 5)



        'TextBox9.Text = Mid((Trim(DataSet21.Tables("ProgramComment").Rows(1).Item(4))), 1, 5) ' job number
        'TextBox10.Text = Mid((Trim(DataSet21.Tables("ProgramComment").Rows(1).Item(4))), 7, 4) ' operation number


        TextBox3.Text = DataSet21.Tables("Load").Rows(2).Item(3)
        Try
            Spindleload = 419 - Int(DataSet21.Tables("Load").Rows(2).Item(3) * 3)
        Catch ex As Exception
            'MsgBox(ex.Message)
        End Try

        TextBox4.Text = DataSet21.Tables("Load").Rows(3).Item(3)
        Xaxisload = 419 - Int(DataSet21.Tables("Load").Rows(3).Item(3) * 3)
        TextBox5.Text = DataSet21.Tables("Load").Rows(4).Item(3)
        Yaxisload = 419 - Int(DataSet21.Tables("Load").Rows(4).Item(3) * 3)
        TextBox6.Text = DataSet21.Tables("Load").Rows(5).Item(3)
        Zaxisload = 419 - Int(DataSet21.Tables("Load").Rows(5).Item(3) * 3)

        TextBox7.Text = DataSet21.Tables("PathFeedrateOverride").Rows(0).Item(4)
        ...
        End Select
        TextBox8.Text = DataSet21.Tables("PathFeedrateOverride").Rows(1).Item(4)
        ...
        End Select
        TextBox13.Text = DataSet21.Tables("EmergencyStop").Rows(0).Item(3)
